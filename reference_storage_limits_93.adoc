...
| Maximum number of data disks across all aggregates ^2^ | 34 for single-node configurations
...
| Maximum number of data disks across all aggregates ^1^ a|
* DS3_v2: 15
* DS4_v2: 31
* DS5_v2: 63
* DS13_v2: 31
...
. Less than 100 TB is supported for this configuration because aggregates are limited to 96 TB of _raw_ capacity. The specified limit is based on the assumption that you use Cloud Manager to create a volume and that you keep the default aggregate free space ratio.
