...
| E4s_v3 | 5 | 160 TB | 2 PB
| E8s_v3 | 13 | 416 TB | 2 PB
...
| DS4_v2 | 29 | 464 TB | 2 PB
| DS5_v2 | 61 | 976 TB | 2 PB
| DS13_v2 | 29 | 464 TB | 2 PB
...
=== Limit for capacity-based licensing and node-based BYOL

Up to 24 storage VMs (SVMs) are supported with Cloud Volumes ONTAP when using capacity-based licensing or node-based BYOL. Those 24 storage VMs can serve data or be configured for disaster recovery (DR). Each storage VM can have up to three LIFs where two are data LIFs and one is an SVM management LIF.
...
==== Multiple storage VMs with capacity-based licensing

There are no extra licensing costs for additional storage VMs, but there is a 4 TB minimum capacity charge per storage VM. For example, if you create two storage VMs and each has 2 TB of provisioned capacity, you'll be charged a total of 8 TB.

==== Multiple storage VMs with node-based BYOL

For a node-based BYOL, an add-on license is required for each additional _data-serving_ storage VM beyond the first storage VM that comes with Cloud Volumes ONTAP by default. Contact your account team to obtain a storage VM add-on license.
