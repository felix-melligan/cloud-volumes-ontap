...
. An HA node can contain up to 18 mirrored aggregates until the 32 data disk limit has been reached. It is not possible to create 18 aggregates on both nodes in an HA pair because it would exceed the 32 data disk limit. Remember that Cloud Manager allocates the same number of disks to both nodes when additional capacity is required.
+
For example, if you created 18 aggregates on node A that contained one disk each, 18 disks would be allocated to both nodes. In an active-active configuration, you could then add up to 14 disks on node B until the 32 data disk limit was reached.
