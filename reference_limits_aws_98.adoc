...
Up to 24 storage VMs are supported with Cloud Volumes ONTAP BYOL when you use a C5, M5, or R5 instance type. But the limit can be lower, depending on the EC2 instance type that you use. The limits per instance are listed in the next section.
...
| Max storage VMs without mgmt LIF ^2,3^
| Max storage VMs with mgmt LIF ^2,3^
...
. A management LIF for the storage VM is optional. Because it requires a private IP address, it will limit the number of additional storage VMs that you can create. The only exception is an HA pair in multiple AZs. In that case, the IP address for the management LIF is a _floating IP address_ so it doesn't count against the _private_ IP limit.
