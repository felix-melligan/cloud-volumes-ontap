...
* c4, m4, and r4 instance types are not supported with new Cloud Volumes ONTAP 9.8 systems. However, we're still showing disk limits for these instance types because you can still upgrade a system to the 9.8 release when running on one of these instance types.
...
| m5d.8xlarge | 16 | 256 TB | 368 TB | 256 TB | 368 TB x each license
