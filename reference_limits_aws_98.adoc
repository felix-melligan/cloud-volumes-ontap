...
Up to 24 storage VMs are supported with Cloud Volumes ONTAP when you use a C5, M5, or R5 instance type and bring your own license (BYOL). Of those 24 storage VMs, up to 12 can be configured for disaster recovery (DR).
...
When you create an additional storage VM, you need to allocate additional private IP addresses on port e0a. The table below identifies the number of private IP addresses that are available for port e0a after Cloud Volumes ONTAP has been deployed. The number of available IP addresses directly affects the maximum number of storage VMs for that configuration.

You'll notice that two storage VM limits are listed for single node systems and for HA pairs in a single AZ. The first number is the max number of storage VMs if you _don't_ allocate a management LIF on each storage VM. The second number is the max number of storage VMs if you _do_ allocate a management LIF on each storage VM.

[cols=4*,options="header"]
|===
| Configuration
| Instance type
| Private IPs remaining
| Max storage VMs
