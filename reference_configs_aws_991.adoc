...
You can pay for Cloud Volumes ONTAP in AWS using the following options.
...
==== FAQ for capacity-based licensing

This FAQ can help if you have more questions about capacity-based licenses.

===== How many Cloud Volumes ONTAP systems can I deploy with a capacity-based license?

Answer: As many as you would like, as long as enough capacity is available from your license.

===== How is charging calculated?

Cloud Manager aggregates the capacity of all data volumes for every Cloud Volumes ONTAP system and charges based on the allocated capacity. Storage efficiency improves total cost of ownership (TCO) since less cloud resources are required, but it doesn't affect license calculations.

===== What is the difference between the Freemium package and a free trial?

A free trial is associated with a PAYGO subscription and is limited to one Cloud Volumes ONTAP system for 30 days on a single cloud provider.

The Freemium package is limited by capacity only (there is no time limit) and provides a full feature set that you can use in all cloud providers, but without technical support. If you're a new customer, you might deploy a dev/test Cloud Volumes ONTAP system using the Freemium plan. When you're ready to use it in full production, you can move to a production-ready, capacity-based charging plan that includes technical support.

===== If I have an AWS contract for the Professional package, can I still deploy Cloud Volumes ONTAP systems using BYOL?

No. Once you have an AWS Marketplace contract, all Cloud Volumes ONTAP systems that you deploy are charged against the contract and not against BYOL.
