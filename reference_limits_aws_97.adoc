...
The following number of SVMs are supported with C5, M5, and R5 instance types when you bring your own license (BYOL) and purchase SVM add-on licenses:

* 12 SVMs with single node systems
* 8 SVMs with HA pairs

NOTE: An SVM spans the entire Cloud Volumes ONTAP system (HA pair or single node)
...
SVMs that you configure for disaster recovery (DR) don't require an SVM add-on license (they are free of charge), but they do count against the SVM limit. ^1,2^b
...
. For example, if you have 8 data-serving SVMs on an HA pair, then you've reached the limit and can't create any additional SVMs. The same is true for another HA pair that has 8 disaster recovery SVMs--you've reached the limit and can't create any additional SVMs.

. You can activate a destination SVM for data access if there’s an outage on the source SVM. Cloud Manager doesn't provide any setup or orchestration support for SVM disaster recovery. You must use System Manager or the CLI.
+
