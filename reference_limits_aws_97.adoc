...
The following number of storage VMs are supported with C5, M5, and R5 instance types when you bring your own license (BYOL) and purchase add-on licenses:
...
An add-on license is required for each additional _data-serving_ SVM beyond the first storage VM that comes with Cloud Volumes ONTAP by default.

Storage VMs that you configure for disaster recovery (DR) don't require an add-on license (they are free of charge), but they do count against the SVM limit. ^1,2^

*All other configurations*
One data-serving SVM and one destination SVM used for disaster recovery are supported. ^2^
