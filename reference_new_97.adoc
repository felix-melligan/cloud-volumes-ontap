...
Multiple SVMs are supported with the C5, M5, and R5 instance types when you bring your own license (BYOL).

The following number of SVMs are supported:

* 12 SVMs with single node systems
* 8 SVMs with HA pairs

An SVM add-on license is required for each additional _data-serving_ SVM beyond the first SVM that comes with Cloud Volumes ONTAP by default.

SVMs that you configure for disaster recovery (DR) don't require an SVM add-on license (they are free of charge), but they do count against the SVM limit.

For example, if you have 8 data-serving SVMs on an HA pair, then you've reached the limit and can't create any additional SVMs. The same is true for another HA pair that has 8 DR SVMs--you've reached the limit and can't create any additional SVMs.

Note that the creation of new SVMs must be done through System Manager or the CLI.

<comment about all other configurations>

== 9.7 P5 (16 July 2020)

Cloud Volumes ONTAP 9.7 P5 is now available in Microsoft Azure. This patch release includes two enhancements.

=== Support for new VM types with Ultra SSD VNVRAM

Cloud Volumes ONTAP now supports the following VM types with single node systems and a Premium or BYOL license:
...
<I don't have any information about this feature.>
...
The 9.7 P4 patch release for Cloud Volumes ONTAP is now available through Cloud Manager 3.8 and later. Cloud Manager will prompt you to upgrade your existing systems to this patch release. https://mysupport.netapp.com/site/products/all/details/cloud-volumes-ontap/downloads-tab/download/62632/9.7P4[View the list of bugs fixed in the P4 patch^].

== 9.7 P3 (2 May 2020)

The 9.7 P3 patch release for Cloud Volumes ONTAP is now available through Cloud Manager 3.8 and later. Cloud Manager will prompt you to upgrade your existing systems to this patch release. https://mysupport.netapp.com/site/products/all/details/cloud-volumes-ontap/downloads-tab/download/62632/9.7P3[View the list of bugs fixed in the P3 patch^].

== 9.7 P2 (8 Apr 2020)

The 9.7 P2 patch release for Cloud Volumes ONTAP is now available through Cloud Manager 3.8 and later. Cloud Manager will prompt you to upgrade your existing systems to this patch release. https://mysupport.netapp.com/site/products/all/details/cloud-volumes-ontap/downloads-tab/download/62632/9.7P2[View the list of bugs fixed in the P2 patch^].
