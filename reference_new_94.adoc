...
You can now reduce your Azure storage costs by combining a performance tier for hot data (Premium or Standard storage) with a capacity tier for cold data (Azure Blob storage). The same tiering policies that are supported in AWS are also supported in Azure: auto, Snapshot only, and backup.
